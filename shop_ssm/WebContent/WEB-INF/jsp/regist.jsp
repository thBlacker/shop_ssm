...
<script>
	...
	function checkUsername(){
		var val = $("#username").val();//获得id为username的value的值
		if(val == "" || $.trim(val)==""){
			$("#span1").html("<span style='color:red'>用户名不能为空！</span>");
		}else{
			var url = "${pageContext.request.contextPath}/checkUsername";
			var args={"username":$.trim(val)};
			$.post(url,args,function(data){
				if(data == "yes"){
					$("#span1").html("<span style='color:red'>用户名已存在！</span>");
				}else{
					$("#span1").html("<span style='color:green'>用户名可以使用！</span>");
				}
			});
		}
	}
	...
	function change(){
		 $("#checkImg").attr("src", $("#checkImg").attr("src")+"?"+Math.random());
	}//点击更换验证码
		...
		<div class="span24">
			<div class="wrap">
				<div class="main clearfix">
					...
					<div>
						<c:if test="${checkCodeError != null }">
							<span style="color: red">${checkCodeError}</span>
						</c:if>
					</div>
							...
							<tbody>
								<tr>
									...
									<td><input type="text" id="username" name="username"
										...
										id="span1"><f:errors path="username" /></span></td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>密&nbsp;&nbsp;码:</th>
									<td><input type="password" id="password" name="password"
										class="text" maxlength="20" autocomplete="off" /> <span><f:errors
								...
								<tr>
									...
									<td><input type="text" id="email" name="email"
										class="text" maxlength="200" /> <span><f:errors
												path="email" /></span></td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>姓名:</th>
									<td><input type="text" name="name" class="text"
										maxlength="200" /> <span><f:errors path="name" /></span></td>
								</tr>
								...
								<tr>
									...
									<td><input type="text" name="addr" class="text"
										maxlength="200" /> <span><f:errors path="addr" /></span></td>
								</tr>
								<tr>
